      ...
      <v-flex xs12>
        <v-card hover>
          <v-card-title>
            ...
            <p class="ml-3 mb-0 font-weight-thin">{{getPost.likes}} LIKES</p>
            <v-spacer></v-spacer>
...
import { GET_POST, ADD_POST_MESSAGE } from "../../gql/queries";
...
export default {
  ...
  data() {
    return {
      ...
      isFormValid: true,
      messageRules: [
        messageBody => !!messageBody || "Message is required"
        // messageBody =>
        //   messageBody.length < 50 || "Message must be less than 50 characters"
      ]
    };
  },
  apollo: {
